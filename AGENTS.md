...
- After finishing work, create or update today's Markdown file in:
  `/Users/cdd/Documents/workspace/obsidian/company/irm-vault/데일리스크럼`
- Use the `YYYY-MM-DD.md` filename format.
- Keep entries concise and grouped by larger work topics first.
- Use numbered top-level items and indented commit-message-style bullets.
- Include affected project names in parentheses when useful, such as `(new-ra-ising)`.
