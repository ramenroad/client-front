...
이 프로젝트의 스타일링 기본 조합은 `twin.macro + emotion`입니다.

### 정적 스타일

정적인 스타일은 `tw`를 많이 사용합니다.
...
props에 따라 스타일이 갈리면 `styled`를 사용합니다.

예:

```ts
const FilterButton = styled.button<FilterButtonProps>(({ active }) => [
  tw`px-12 py-4 rounded-50 font-14-r`,
...
기존 컴포넌트를 확장할 때는 `tw(Component)` 패턴도 사용합니다.
...
6. 스타일은 `twin.macro + emotion` 조합으로 작성한다.
7. 정적 스타일은 `tw`, 조건부 스타일은 `styled`를 우선 사용한다.
