...
const MenuBoardDetailWrapper = render.div(
  "relative w-240 h-84 bg-white bg-opacity-[0.12] rounded-[8px] flex flex-col justify-center gap-10 px-20 py-16 text-black font-14-m box-border",
);
...
const ProfileImage = render.img("w-24 h-24 rounded-full");

const Divider = render.div("w-1 h-10 bg-gray-800");

const Nickname = render.div("font-14-r text-gray-200 leading-18 w-60 truncate");

const CreatedAt = render.div("font-12-l text-gray-400");

const Description = render.div("font-14-r text-white truncate");

const Tooltip = render.div(
  "absolute mt-4 w-48 h-37 bg-gray-700 rounded-[8px] flex items-center justify-center z-10 left-[-24px]",
);

const TooltipText = render.div("text-gray-200 font-12-m");
