...
export const CommunityBoardCard = ({ board }: CommunityBoardCardProps) => {
  ...
  const authorProfileImageUrl = board.userId?.profileImageUrl || profileDefaultImage;
...
const CategoryChip = render.div(
  "mb-8 inline-flex items-center rounded-[4px] bg-border px-8 py-[2px] text-[11px] leading-[17px] font-medium text-gray-500",
);

const Title = render.div("font-16-m text-gray-900");

const Description = render.div("mt-4 font-14-r text-gray-500 break-words");

const AuthorRow = render.div("mt-14 flex items-center gap-6");

const Avatar = render.img("h-20 w-20 rounded-full bg-border object-cover");

const AuthorName = render.div("text-[13px] leading-[20px] font-medium text-gray-500");
...
const ThumbnailFrame = render.div(
  "relative h-80 w-80 shrink-0 overflow-hidden rounded-[8px] border border-solid border-border bg-border",
);
