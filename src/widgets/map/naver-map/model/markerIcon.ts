...
} satisfies Record<'inactive' | 'normal' | 'selected', MarkerDimensions>
...
const INACTIVE_SELECTED_MARKER_SVG = SELECTED_MARKER_SVG.replace('fill="#FF5E00"', 'fill="#838383"')
...
const getMarkerDimensions = ({ inactive, isSelected }: { inactive?: boolean; isSelected: boolean }) => {
  if (inactive && isSelected) {
    return MARKER_DIMENSIONS.selected
  }

  if (inactive) {
    return MARKER_DIMENSIONS.inactive
  }

  return isSelected ? MARKER_DIMENSIONS.selected : MARKER_DIMENSIONS.normal
}

const getMarkerSvg = ({ inactive, isSelected }: { inactive?: boolean; isSelected: boolean }) => {
  if (inactive && isSelected) {
    return INACTIVE_SELECTED_MARKER_SVG
  }
...
export const createRamenyaMarkerIcon = ({
  ...
  const { anchorX, anchorY, height, width } = getMarkerDimensions({ inactive, isSelected })
  const renderKey = `${inactive && isSelected ? 'inactive-selected' : inactive ? 'inactive' : isSelected ? 'selected' : 'normal'}:${zoom ?? 'initial'}`

  return {
    content: `
      <div
        data-marker-render-key="${escapeHtml(renderKey)}"
        style="position: relative; width: ${width}px; height: ${height}px; overflow: visible; transform: translateZ(0);"
      >
...
export const createCurrentLocationMarkerIcon = (maps: NaverMaps): NaverMarkerIcon => ({
  content: `
    <div style="position: relative; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
      <div style="position: absolute; width: 40px; height: 40px; border-radius: 50%; background: rgba(66, 133, 244, 0.3); animation: raising-current-location-pulse 2s infinite;"></div>
      <div style="position: absolute; width: 32px; height: 32px; border-radius: 50%; background: rgba(66, 133, 244, 0.2); animation: raising-current-location-pulse 2s infinite 0.5s;"></div>
      <div style="position: absolute; width: 24px; height: 24px; border-radius: 50%; background: rgba(66, 133, 244, 0.1); animation: raising-current-location-pulse 2s infinite 1s;"></div>
    ...
    <style>
      @keyframes raising-current-location-pulse {
