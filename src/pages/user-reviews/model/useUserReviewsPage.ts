...
export const useUserReviewsPage = () => {
  ...
  const isAuthSettled =
    !authSession.isSignIn || myInfoQuery.isFetched || myInfoQuery.isError;
  const isMine = Boolean(
    authSession.isSignIn && userId && myInfoQuery.data?._id === userId,
  );
  ...
  const share = useShare({
    title: userInfoQuery.data?.nickname
      ? `${userInfoQuery.data.nickname}님의 페이지`
      : "라이징",
    description: `${userInfoQuery.data?.nickname ?? ""}님의 페이지를 확인해보세요!`,
    text: `${userInfoQuery.data?.nickname ?? ""}님의 페이지를 확인해보세요!`,
    imageUrl: userInfoQuery.data?.profileImageUrl,
    buttonTitle: "리뷰 보러가기",
  });
  const myReviewQuery = useMyReviewsInfiniteQuery(
    { limit: 10 },
    { enabled: isMine },
  );
  const userReviewQuery = useUserReviewsInfiniteQuery(
    userId,
    { limit: 10 },
    {
  ...
  const updateIsPublicMutation = useUpdateIsPublicMutation({
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: reviewQueryKeys.myReviews({ limit: 10 }),
      });
    },
  });
  ...
  const isFetchingNextPage = isMine
    ? myReviewQuery.isFetchingNextPage
    : userReviewQuery.isFetchingNextPage;
  const hasNextPage = isMine
    ? myReviewQuery.hasNextPage
  ...
  return {
    ...
    isLoading:
      ...
      (isMine ? myReviewQuery.isLoading : userReviewQuery.isLoading),
