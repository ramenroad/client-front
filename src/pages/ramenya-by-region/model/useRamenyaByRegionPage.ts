...
import { filterRamenyas, initialFilterOptions, SortType, type FilterOptions } from '@/entities/ramenya/model'
import { getBrowserCurrentLocation } from '@/shared/lib/geolocation'
import type { Coordinate } from '@/shared/lib/naver-map'

const LOCATION_FILTER_STORAGE_KEY = 'locationPageFilterOptions'
...
const readStoredFilterOptions = () => {
  ...
  try {
    const rawValue = window.sessionStorage.getItem(LOCATION_FILTER_STORAGE_KEY)
...
const writeStoredFilterOptions = (filterOptions: FilterOptions) => {
  ...
  window.sessionStorage.setItem(LOCATION_FILTER_STORAGE_KEY, JSON.stringify(filterOptions))
}

export const useRamenyaByRegionPage = () => {
  const navigate = useNavigate()
  const { location = '' } = useParams()
  const [filterOptions, setFilterOptions] = useState(readStoredFilterOptions)
  const [currentLocation, setCurrentLocation] = useState<Coordinate | null>(null)
  const ramenyaListQuery = useRamenyaListQuery({ region: location }, { enabled: Boolean(location) })
