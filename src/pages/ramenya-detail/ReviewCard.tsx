...
export const ReviewCard = ({ review }: { review: UserReview<ReviewType.USER> }) => {
  ...
  const { ramenyaDetailQuery } = useRamenyaDetailQuery(review.ramenyaId);
  const { refetch: refetchRamenyaDetail } = ramenyaDetailQuery;
  ...
  const displayText = isTextLong && !isExpanded ? `${review.review.slice(0, MAX_TEXT_LENGTH)}...` : review.review;
        ...
        <ReviewHeader>
          <ReviewNameBox>
            ...
            <ReviewerInfoBox>
              ...
              <ReviewerReviewInfo>
                ...
                <ReviewerReviewCount>리뷰 {review.userId.reviewCount}</ReviewerReviewCount>
              </ReviewerReviewInfo>
            </ReviewerInfoBox>
          </ReviewNameBox>
          ...
          {review.userId._id === userInformationQuery.data?._id && (
            <ButtonContainer>
              ...
              <ReviewButton onClick={handleOpenDeleteModal}>삭제</ReviewButton>
            </ButtonContainer>
          )}
        </ReviewHeader>
          ...
          <ScoreBox>
            ...
            <ScoredMenuContainer>
              {Array.isArray(review.menus) &&
                review.menus.map((menu: string, index: number) => (
                  <React.Fragment key={menu}>
                    <ScoredMenu>{menu}</ScoredMenu>
                    {index < review.menus.length - 1 && <MenuDivider />}
                  </React.Fragment>
                ))}
            </ScoredMenuContainer>
          </ScoreBox>
          <ReviewDate>{formatDate(review.createdAt.toString())}</ReviewDate>
        </ReviewScore>

        <ReviewDetail>
          <ReviewTextContainer>
            <span>{displayText}</span>
            {isTextLong && <MoreButton onClick={toggleExpand}>{isExpanded ? "접기" : "더보기"}</MoreButton>}
          </ReviewTextContainer>
        </ReviewDetail>
